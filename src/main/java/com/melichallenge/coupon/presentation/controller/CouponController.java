...
public class CouponController {
    ...
    ResponseEntity<MaximizedTotalToSpendDto> maximizeCouponBenefit(
            @RequestBody @Valid FavouritesClientItemsWithCouponDto request
    ) throws BusinessException {

        // Calling service.
        MaximizedTotalToSpend maximizedTotalToSpent = maximizeCouponUseService
                .maximize(request.itemsIds(), request.amount());
    ...
    ResponseEntity<List<BestRedeemedProductsDto>> findTopRedeemedProducts() {
        ...
        return ResponseEntity.ok(
                bestRedeemedProductsService.findTopRedeemedProducts().stream() // Calling Service
                        .map(b -> new BestRedeemedProductsDto(b.getProductName(), b.getQuantity())) // Mapping to DTO.
                        .toList());
    }
}
