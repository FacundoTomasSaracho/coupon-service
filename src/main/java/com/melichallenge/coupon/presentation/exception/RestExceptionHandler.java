...
public class RestExceptionHandler {
    ...
    public ResponseEntity<ErrorDto> handleException(Exception exception) {
        log.error(exception);
        return new ResponseEntity<>(new ErrorDto(5000, "Server error. Please contact support team."), HttpStatus.INTERNAL_SERVER_ERROR);
    }

    @ExceptionHandler(HttpMessageNotReadableException.class)
    public ResponseEntity<ErrorDto> handleException(HttpMessageNotReadableException exception) {
        log.error(exception);
        return new ResponseEntity<>(new ErrorDto(400, "Bad request."), HttpStatus.BAD_REQUEST);
    }
